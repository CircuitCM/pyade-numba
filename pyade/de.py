...
def apply_de(population_size: int,
             individual_size: int,
             f: Union[float, int],
             cr: Union[float, int],
             bounds: A,
             #cross: str,#for now its only bin. can add others if helpful in the future.
             max_evals: int,
             pop_eval: Callable,
             *eval_opts) -> A:
    """
    ...
    _t_pop=np.empty((nb.get_num_threads(),population.shape[1]),dtype=np.float64)
    _idx = np.empty((nb.get_num_threads(), population.shape[0]), dtype=np.int64)
...
def _de_bin_mutate_bc(population: A,
    ...
    for current_generation in range(max_iters):
        cmn.uchoice_mutator(population, _m_pop, bounds, enf_bounds, cfg.BIN_MUTATE_R, cmn.bin_mutate, _idx, _crr, _t_pop, tf, cr, rand.randint(0, population.shape[0]))
...
def _de_c_t_best2_bc(population: A,
    ...
    _b = np.argmin(_ftdiym)
    for current_generation in range(max_iters):
        cmn.uchoice_mutator(population, _m_pop, bounds, enf_bounds, cfg.C_T_B2_MUTATE_R, cmn.c_to_best_2_bin_mutate, _idx, _crr, _t_pop, _b, tf, cr)
        tf=_the_rest(population,_m_pop,_ftdiym,_idxs,f,pop_eval,*eval_opts)
        _b = np.argmin(_ftdiym)
    return population[_b], _ftdiym[_b]
...
def _de_c_t_pbest_bc(population: A,
                            bounds: A, enf_bounds:bool, #Enforce bounds
                            max_iters:int,
                            f: float, p:int, cr: float, #if p is -1 then jitter along the best half
                            seed: int,
                            pop_eval: Callable,
                            _m_pop:A,_idx:A,_crr:A,_t_pop:A,_ftdiym:A, _idxs:A,
                            *eval_opts) -> tuple[A,A]:
    ...
    for current_generation in range(max_iters):
        tp=_pset(p,psz)
        cmn.uchoice_mutator(population, _m_pop, bounds, enf_bounds, cfg.C_T_PB_MUTATE_R, cmn.c_to_pbest_mutate, _idx, _crr, _t_pop, _b, tp, tf, cr, rand.randint(0, population.shape[0]))
        tf=_the_rest(population,_m_pop,_ftdiym,_idxs,f,pop_eval,*eval_opts)
